...
def validate_creds(username, password,ldap_server):
...
def brute_force(user_list, password_array, server_list):
    for user in user_list:
        ...
        while len(passwords) > 0:
            ...
            if validate_creds(user,password,server):
                with open("Results.txt", "a") as myfile:
                    myfile.write(user+" has password "+ password + "\n")

def password_spray(user_list, password_array, server_list):
